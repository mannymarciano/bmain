...
RETURNS TRIGGER AS $$
BEGIN
  -- Delete the file from storage using correct signature
  PERFORM storage.delete(ARRAY[OLD.file_path], 'backups');
  RETURN OLD;
END;
...
-- Update storage policies
CREATE OR REPLACE POLICY "Users can delete their backup files"
